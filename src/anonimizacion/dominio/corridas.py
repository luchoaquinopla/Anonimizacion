"""Máquina de transiciones de corrida y documento.
Ver `estados_corrida.py` para el contexto de por qué existe."""
...
# `CREADA -> FALLIDA` cubre recuperación de arranque; `PROCESANDO -> {COMPLETADA,
# COMPLETADA_CON_CUARENTENA}` es directa porque `procesar_grupo` ya reconcilia y
# publica cada grupo de punta a punta en una sola llamada síncrona.
_TRANSICIONES_CORRIDA = {
    ...
    # Única salida de un estado terminal: reintentar reencola bajo el mismo corrida_id.
    # COMPLETADA no tiene salida a propósito: cero apartados, nada que reintentar.
...
class Corrida:
    ...
    version: int = 0
    # Raíz autorizada de la corrida; None por compatibilidad con fixtures/llamadores previos.
