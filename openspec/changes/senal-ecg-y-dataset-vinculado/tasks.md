...
- [x] 4.3 **RED/GREEN**: ejecutar el verificador sobre la exportación del corpus sintético (PR 3) y sobre el banco completo; probar 0 coincidencias de nombre, DNI y fecha de nacimiento (criterio de éxito de la propuesta); documentar el resultado en `docs/pipeline.md`.
  ...
  - Corrección de revisión: `tests/salida/test_esquema_arrow_de_exportacion.py` (nuevo) fija el esquema Arrow EXACTO (nombre, tipo y orden) de los 4 Parquet con tipos literales (nunca importados de `exportacion.py`, para no espejar) y agrega el test de regresión del bug de pyarrow (página con todos los ECG sin señal, round-trip verificado).
