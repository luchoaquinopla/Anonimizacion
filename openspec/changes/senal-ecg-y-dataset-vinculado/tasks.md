...
- [ ] 1.1 **RED**: crear `tests/extraccion/test_trazos_pymupdf.py` con un PDF sintético mínimo rotado 90° que dibuja un rectángulo conocido; fijar si `get_drawings()` devuelve coordenadas rotadas o sin rotar respecto de `page.derotation_matrix` (pregunta abierta del diseño, decisión #1 del algoritmo).
- [ ] 1.2 **GREEN**: crear `src/anonimizacion/extraccion/trazos_pymupdf.py`: captura de trazos negros (0,0,0) sin relleno vía `get_drawings()`, proyección a espacio sin rotar y a mm, según lo fijado en 1.1.
